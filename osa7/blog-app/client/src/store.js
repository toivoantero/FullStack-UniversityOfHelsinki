...
const useBlogStore = create((set, get) => ({
  ...
  actions: {
    ...
    comment: async (comment) => {
      ...
      const savedComments = await commentService.getAll();
      const savedComment = savedComments[savedComments.length - 1];
      set((state) => {
        const blogToBeCommented = state.blogs.find(
          (b) => b.id === comment.blog.id,
        );
        const commentedBlog = {
          ...blogToBeCommented,
          comments: [...blogToBeCommented.comments, savedComment],
        };
        return {
          blogs: state.blogs.map((b) =>
            b.id === comment.blog.id ? commentedBlog : b,
          ),
        };
      });
    },
...
const useNotificationStore = create((set) => ({
  ...
  actions: {
    setNotification: (notificationContent) =>
      set(() => ({ notification: notificationContent })),
  },
}));
